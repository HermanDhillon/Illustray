...
module.exports = {
  ...
  create: async (username, email, hash) => {
    try {
      if (username && email && hash) {
        const result = await pgPool.query('INSERT INTO users (username, email, hash) VALUES ($1, $2, $3) RETURNING *', [username, email, hash]);
  ...
  findByEmailOrUsername: async (email, username) => {
    try {
      if (email) {
        const result = await pgPool.query('SELECT * from users WHERE email=$1 OR username=$2', [email, username]);
        return result.rows[0]; // returns a single object
      }
      console.error('Email is a required input.');
    } catch (err) {
  ...
  findMany: async () => {
    try {
      const result = await pgPool.query('SELECT * from users ORDER BY username ASC');
